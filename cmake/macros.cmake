...
MACRO(ROBOT_MODULE)
	SET(RBM_SYNTAX "NAME,1,1,RBM_HAS_NAME,ROBOTNAME")
	...
	IF(WIN32)
		IF(NOT RBM_HAS_INTERFACE)
			SET(RBM_INTERFACE "LEGACY_MIN")
		ENDIF(NOT RBM_HAS_INTERFACE)
		SET(ROBOT_DEF_FILE ${CMAKE_CURRENT_BINARY_DIR}/${ROBOTNAME}_gen.def)
		GENERATE_ROBOT_DEF_FILE(${ROBOTNAME} ${ROBOT_DEF_FILE} ${RBM_INTERFACE})
	...
	ADD_LIBRARY(${ROBOTNAME} SHARED ${RBM_SOURCES})
	IF(UNIX)
...
MACRO(ROBOT_DATA)
	SET(RBD_SYNTAX "NAME,1,1,RBD_HAS_NAME,ROBOTNAME")
	...
	#MESSAGE(STATUS "ROBOT_DATA(${ROBOTNAME}): PREFIX=${RBD_PREFIX} FILES=${RBD_FILES} SUBDIRS=${RBD_SUBDIRS}")

	IF(NOT RBD_HAS_NAME)
		MESSAGE(FATAL_ERROR "Cannot install data for a robot module with no specified name")
	ENDIF(NOT RBD_HAS_NAME)
	...
	IF(RBD_HAS_FILES)
		...
		#MESSAGE(STATUS "Files to install(-1) for ${ROBOTNAME} robot: ${REAL_FILES}")
	ENDIF(RBD_HAS_FILES)
...
#  SUBDIRS	: Data subdirectories to install in the robot's data dir
MACRO(ROBOT)
	SET(RB_SYNTAX "NAME,1,1,RB_HAS_NAME,ROBOTNAME")
	...
	#MESSAGE(STATUS "ROBOT(${ROBOTNAME}): INTERFACE=${RB_INTERFACE} (${RB_HAS_INTERFACE}) SOURCES=${RB_SOURCES} (${RB_HAS_SOURCES}) PREFIX=${RB_PREFIX} (${RB_HAS_PREFIX}) FILES=${RB_FILES} (${RB_HAS_FILES}) SUBDIRS=${RB_SUBDIRS} (${RB_HAS_SUBDIRS})")

	IF(NOT RB_HAS_NAME)
		MESSAGE(FATAL_ERROR "Cannot build a robot with no specified name")
	ENDIF(NOT RB_HAS_NAME)

	IF(NOT RB_HAS_SOURCES)
		MESSAGE(FATAL_ERROR "Cannot build a robot without sources")
	ENDIF(NOT RB_HAS_SOURCES)

	IF(RB_HAS_INTERFACE)
		ROBOT_MODULE(NAME ${ROBOTNAME} INTERFACE ${RB_INTERFACE} SOURCES ${RB_SOURCES})
	ELSE(RB_HAS_INTERFACE)
		ROBOT_MODULE(NAME ${ROBOTNAME} SOURCES ${RB_SOURCES})
	ENDIF(RB_HAS_INTERFACE)

	#TODO: Make all this stuff only one line ?
	IF(RB_HAS_PREFIX)
		IF(RB_HAS_FILES)
			IF(RB_HAS_SUBDIRS)
				ROBOT_DATA(NAME ${ROBOTNAME} PREFIX ${RB_PREFIX} FILES ${RB_FILES} SUBDIRS ${RB_SUBDIRS})
			ELSE(RB_HAS_SUBDIRS)
				ROBOT_DATA(NAME ${ROBOTNAME} PREFIX ${RB_PREFIX} FILES ${RB_FILES})
			ENDIF(RB_HAS_SUBDIRS)
		ELSE(RB_HAS_FILES)
			IF(RB_HAS_SUBDIRS)
				ROBOT_DATA(NAME ${ROBOTNAME} PREFIX ${RB_PREFIX} SUBDIRS ${RB_SUBDIRS})
			ELSE(RB_HAS_SUBDIRS)
				ROBOT_DATA(NAME ${ROBOTNAME} PREFIX ${RB_PREFIX})
			ENDIF(RB_HAS_SUBDIRS)
		ENDIF(RB_HAS_FILES)
	ELSE(RB_HAS_PREFIX)
		IF(RB_HAS_FILES)
			IF(RB_HAS_SUBDIRS)
				ROBOT_DATA(NAME ${ROBOTNAME} FILES ${RB_FILES} SUBDIRS ${RB_SUBDIRS})
			ELSE(RB_HAS_SUBDIRS)
				ROBOT_DATA(NAME ${ROBOTNAME} FILES ${RB_FILES})
			ENDIF(RB_HAS_SUBDIRS)
		ELSE(RB_HAS_FILES)
			IF(RB_HAS_SUBDIRS)
				ROBOT_DATA(NAME ${ROBOTNAME} SUBDIRS ${RB_SUBDIRS})
			ELSE(RB_HAS_SUBDIRS)
