...
void PackedBuffer::pack_ubyte(unsigned char v)
{
    ...
    {
        GfLogError("pack_ubyte: buffer overflow: buf_size=%zu "
...
void PackedBuffer::pack_short(short v)
{
    ...
    if (bounds_error(size))
    {
        GfLogError("pack_short: buffer overflow: buf_size=%zu "
                "data_length=%zu\n", buf_size, data_length);
        throw PackedBufferException();
    }
...
void PackedBuffer::pack_int(int v)
{
    ...
    if (bounds_error(size))
    {
        GfLogError("pack_int: buffer overflow: buf_size=%zu "
                "data_length=%zu\n", buf_size, data_length);
        throw PackedBufferException();
    }
...
void PackedBuffer::pack_ushort(unsigned short v)
{
    ...
    if (bounds_error(size))
    {
        GfLogError("pack_ushort: buffer overflow: buf_size=%zu "
                "data_length=%zu\n", buf_size, data_length);
        throw PackedBufferException();
    }
...
void PackedBuffer::pack_uint(unsigned int v)
{
    ...
    if (bounds_error(size))
    {
        GfLogError("pack_uint: buffer overflow: buf_size=%zu "
                "data_length=%zu\n", buf_size, data_length);
        throw PackedBufferException();
    }
...
void PackedBuffer::pack_float(float v)
{
    ...
    if (bounds_error(size))
    {
        GfLogError("pack_float: buffer overflow: buf_size=%zu "
                "data_length=%zu\n", buf_size, data_length);
        throw PackedBufferException();
    }
...
void PackedBuffer::pack_double(double v) { const size_t size = sizeof(Uint64);
    ...
    if (bounds_error(size))
    {
        GfLogError("pack_double: buffer overflow: buf_size=%zu "
                "data_length=%zu\n", buf_size, data_length);
        throw PackedBufferException();
    }
...
void PackedBuffer::pack_vector(const float *v)
{
    ...
    if (bounds_error(size))
    {
        GfLogError("pack_vector: buffer overflow: buf_size=%zu "
                "data_length=%zu\n", buf_size, data_length);
        throw PackedBufferException();
    }
...
void PackedBuffer::pack_string(const void *m, int len)
{
    if (bounds_error(len))
    {
        GfLogError("pack_string: buffer overflow: buf_size=%zu "
                "data_length=%zu\n", buf_size, data_length);
        throw PackedBufferException();
    }
...
void PackedBuffer::pack_stdstring(const std::string& str)
{
    ...
    if (bounds_error(size))
    {
        GfLogError("pack_stdstring: buffer overflow: buf_size=%zu "
                "data_length=%zu\n", buf_size, data_length);
        throw PackedBufferException();
    }
...
unsigned char PackedBuffer::unpack_ubyte()
{
    ...
    if (bounds_error(size))
    {
        GfLogError("unpack_ubyte: buffer overrun: buf_size=%zu "
                "data_length=%zu\n", buf_size, data_length);
        throw PackedBufferException();
    }
...
short PackedBuffer::unpack_short()
{
    ...
    if (bounds_error(size))
    {
        GfLogError("unpack_short: buffer overrun: buf_size=%zu "
                "data_length=%zu\n", buf_size, data_length);
        throw PackedBufferException();
    }
...
int PackedBuffer::unpack_int()
{
    ...
    if (bounds_error(size))
    {
        GfLogError("unpack_int: buffer overrun: buf_size=%zu "
                "data_length=%zu\n", buf_size, data_length);
        throw PackedBufferException();
    }
...
unsigned short PackedBuffer::unpack_ushort()
{
    ...
    if (bounds_error(size))
    {
        GfLogError("unpack_ushort: buffer overrun: buf_size=%zu "
                "data_length=%zu\n", buf_size, data_length);
        throw PackedBufferException();
    }
...
unsigned int PackedBuffer::unpack_uint()
{
    ...
    if (bounds_error(size))
    {
        GfLogError("unpack_uint: buffer overrun: buf_size=%zu "
                "data_length=%zu\n", buf_size, data_length);
        throw PackedBufferException();
    }
...
float PackedBuffer::unpack_float()
{
    ...
    if (bounds_error(size))
    {
        GfLogError("unpack_float: buffer overrun: buf_size=%zu "
                "data_length=%zu\n", buf_size, data_length);
        throw PackedBufferException();
    }
...
double PackedBuffer::unpack_double()
{
    ...
    if (bounds_error(size))
    {
        GfLogError("unpack_double: buffer overrun: buf_size=%zu "
                "data_length=%zu\n", buf_size, data_length);
        throw PackedBufferException();
    }
...
float *PackedBuffer::unpack_vector(float *v)
{
    ...
    if (bounds_error(size))
    {
        GfLogError("unpack_vector: buffer overrun: buf_size=%zu "
                "data_length=%zu\n", buf_size, data_length);
        throw PackedBufferException();
    }
...
void *PackedBuffer::unpack_string(void *m, int len)
{
    if (bounds_error(len))
    {
        GfLogError("unpack_string: buffer overrun: buf_size=%zu "
                "data_length=%zu\n", buf_size, data_length);
        throw PackedBufferException();
    }
...
std::string &PackedBuffer::unpack_stdstring(std::string& str)
{
    ...
    if (bounds_error(size))
    {
        GfLogError("unpack_stdstring: packed overrun: buf_size=%zu "
                "data_length=%zu\n", buf_size, data_length);
        throw PackedBufferException();
    }
