...
grCustomizePits(void)
{
	...
	switch (pits->type) {
		...
		case TR_PIT_NO_BUILDING:
			...
			ssgState *stWall = grSsgLoadTexStateEx(strWallFileName.c_str(), buf, FALSE, FALSE, FALSE);
			((ssgSimpleState*)stWall)->setShininess(100);
			...
			for(int i = 0; i < pits->nMaxPits; i++) {
				...
				//This offset needed so the pit walls start at the correct plaze
				...
				ssgVtxTable *pit = new ssgVtxTable(GL_TRIANGLE_STRIP, pit_vtx1, pit_nrm1, pit_tex1, pit_clr1);
				pit->setState(stWall);
				pit->setCullFace(0);
				ThePits->addKid(pit);
				...
				//Load logo texture (.rgb first, for backwards compatibility, then .png)
				const std::string strRGBLogoFileName = strLogoFileName + ".rgb";
				ssgState *stLogo = grSsgLoadTexStateEx(strRGBLogoFileName.c_str(), buf, FALSE, FALSE, FALSE);
				if (!stLogo) {
					const std::string strPNGLogoFileName = strLogoFileName + ".png";
					stLogo = grSsgLoadTexStateEx(strPNGLogoFileName.c_str(), buf, FALSE, FALSE);
				}
				((ssgSimpleState*)stLogo)->setShininess(50);
				...
				ssgVtxTable *pit2 = new ssgVtxTable(GL_TRIANGLE_STRIP, pit_vtx2, pit_nrm2, pit_tex2, pit_clr2);
				...
				ssgVtxTable *pit3 = new ssgVtxTable(GL_TRIANGLE_STRIP, pit_vtx3, pit_nrm3, pit_tex3, pit_clr3);
