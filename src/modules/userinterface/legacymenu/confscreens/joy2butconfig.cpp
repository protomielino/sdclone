...
static const char *Instructions[] = {
    "Place the joystick in the NULL position then press a button",
    "Place the joystick in the ATOB position then press a button",
    "Press a button to compute boundaries",
...
static void advanceStep (void)
{
	...
	for (AtobCount = 0; AtobCount <= 23; AtobCount++) {
	    if (Cmd[AtobCount].ref.index == AtobAxis) {
...
JoyCalAutomaton(void)
{
	...
	AtobList->next = NULL;
	AtobList->command = -1;
	AtobList->value = JoyAxis[AtobAxis];
