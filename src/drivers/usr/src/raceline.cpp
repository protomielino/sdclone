...
double LRaceLine::getCornerSpeed(int Div, int rl)
{
    ...
    if (rl == LINE_RL)
    {
        if (overrideCollection)
        {
            ...
            else if (cardata->aFTT < 0.70)
                coldfactor = MIN(1.0, MAX(0.4, (cardata->aFTT)));

            if (cardata->aFTT < 0.8)
                return cornerspeed_cold_override * CornerSpeedFactor;
            else if (cardata->aFTT < 0.7)
    ...
    if (overrideCollection)
    {
        ...
        if (labelOverride)
        {
            ...
            else
            {
                ...
                if (cardata->HasTYC == TRUE && cardata->aFTT < 0.8)
                    coldfactor = MIN(1.0, brakedelay_cold_override / brakedelay_override);
                else if (cardata->HasTYC == TRUE && cardata->aFTT < 0.7)
...
void LRaceLine::InitTrack(tTrack* track, tSituation *p)
{
    ...
    apex = GfParmGetNum(car->_carHandle, SECT_PRIVATE, PRV_APEX, (char*)NULL, 1.0f);
    correctFactor = GfParmGetNum(car->_carHandle, SECT_PRIVATE, PRV_CORRECT_FACTOR, (char*)NULL, 1.0f);
    ...
    lookAheadIn = GfParmGetNum(car->_carHandle, SECT_PRIVATE, PRV_LOOKAHEAD_IN, (char*)NULL, -1.0f);
    ...
    steerSmooth = (int)GfParmGetNum(car->_carHandle, SECT_PRIVATE, PRV_STEER_SMOOTH, (char*)NULL, 1.0f);
    ...
    pitExitTI = GfParmGetNum(car->_carHandle, SECT_PRIVATE, PRV_PIT_EXIT_TI, (char *)NULL, (tdble) 1.0f);
    hasLastUpdate = false;
    ...
    if (lastUpdateDist > 0.0)
    {
        ...
        else if (cardata->HasTYC == true)
        {
            shouldUpdate = (fabs(car->_fuel - cardata->fuel) > (raceType == RM_TYPE_QUALIF ? 1.0 : 5.0) ||
                ...
                (/*(CaTT() - */cardata->aTT < 0.8 || cardata->aTT /*- CaTT()*/ > 0.9) && (cardata->lTT < cardata->CTTT + 0.50));
        }
        else
...
void LRaceLine::ComputeRacelineBraking(int i, int rl, double **tSpeed, int speedrl)
{
    ...
    if (cardata->aFTT < 0.80)
        TireAccel *= MAX(0.65, MIN(1.0, (0.65 * cardata->aTT) + 0.45));
...
void LRaceLine::ComputeRacelineSpeed(int i, int rl, double **tSpeed, int speedrl)
{
    ...
    if (cardata->HasTYC)
    {
        ...
        {
            if (rInverse < 0.0)
                MaxSpeed = (MaxSpeed /*- 20.0)*/ * cardata->lmTT);
            else if (rInverse > 0.0)
                MaxSpeed = (MaxSpeed /*- 20.0) */* cardata->rmTT);
        }
        else
        {
            if (MaxSpeed > 25.0 && fabs(rInverse) > 0.0005)
