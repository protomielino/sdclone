...
bool Driver::updateOnCollision()
{
    ...
    /*
  // Braking test
  if (mCar.car()->race.laps == 4 ) {
    if (mFromStart > 2550.0) {
      m[COLL] = true;
      std::cout << "Braking test fs=" << mFromStart << " speed=" << mCar.v() * 3.6 << " bd=" << brakeDist(mDrvPath, mCar.v(), 0.0) << std::endl;
    } else {
      std::cout << "fs=" << mFromStart << std::endl;
    }
  }
*/
    return m[COLL];
