/***************************************************************************
    ...
    version              : $Id: usr_ls1.cpp,v 1.1 2008/02/11 00:53:10 andrew Exp $
...
extern "C" int moduleWelcome(const tModWelcomeIn* welcomeIn, tModWelcomeOut* welcomeOut)
{
    ...
    if (strncmp(robotName,"usr_ls1",strlen("usr_ls1")) == 0)
		SetUpUSR_ls1();
	else 
		SetUpUSR();
...
extern "C" int usr_ls1(tModInfo *modInfo)
{
  ...
  snprintf(pathBuffer, BUFSIZE, "drivers/usr_ls1/usr_ls1.xml");
  snprintf(nameBuffer, BUFSIZE, "usr_ls1");
