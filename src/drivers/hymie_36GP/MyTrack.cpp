...
void	MyTrack::NewTrack( tTrack* pNewTrack, CarModel *cm, bool pit, SideMod* pSideMod )
{
...
//			m_pSegs[i].wr *= 0.6;
			...
			m_pSegs[i].wl -= cm->LFT_MARGIN;
			m_pSegs[i].wr -= cm->RGT_MARGIN;

			for (int pm = 0; pm < cm->m_pmused; pm++)
			{
				if (i >= cm->m_pm[pm].bgn_div && i <= cm->m_pm[pm].end_div)
				{
					m_pSegs[i].wl -= cm->m_pm[pm].LFT_MARGIN;
					m_pSegs[i].wr -= cm->m_pm[pm].RGT_MARGIN;
					break;
				}
			}
