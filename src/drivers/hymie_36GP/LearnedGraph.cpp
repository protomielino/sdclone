...
{
	return m_pData[MAX(0, MIN(m_dataSize-1, index))];
}
...
double	LearnedGraph::GetValue( const int* index ) const
{
	...
	return m_pData[MAX(0, MIN(m_dataSize-1, idx))];
}
...
double	LearnedGraph::CalcValue( int dim, int offs, const Idx* idx ) const
{
	if( dim < m_nAxes && dim >= 0 )
	{
		...
		return m_pData[MAX(0, MIN(m_dataSize-1, offs))];
}
...
void	LearnedGraph::LearnValue( int dim, int offs, const Idx* idx, double delta )
{
	...
	else if (offs >= 0 && offs < m_dataSize)
		m_pData[offs] += delta;
}
