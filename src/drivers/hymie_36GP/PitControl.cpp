...
int	PitControl::Process( CarElt* pCar, int pitInUse, CarElt *oCar )
{
	//
	//	update statistics.
	//
...
//	bool	likeToPit = pCar->_dammage + damagePerLap >= repairLimit ||
	bool	likeToPit = pCar->_dammage >= repairLimit ||
						pCar->_fuel < fuelPerLap * (oCar == NULL ? 1.5 : 2.5);
//	likeToPit = true;
...
//	GfOut( "******* remaining laps %d\n", pCar->race.remainingLaps );
	...
	if( remainingLaps <= 2 && 
	    pCar->_dammage < 8800 &&
	    pCar->_fuel > (fuelPerLap * pCar->_remainingLaps) - 0.4)
	{
		likeToPit = false;
	}
	...
	if (pitInUse == 2)
	{
		...
		// override other car's pit decision if we have critical fuel...
		else if (pCar->_fuel < oCar->_fuel && pCar->_fuel < fuelPerLap * 2.5)
			pitInUse = 0;
	}

	if (!pitInUse || m_state == PIT_EXIT)
	{
		//
...
//					pCar->ctrl.brakeCmd = 0.5;
				pCar->_accelCmd = MIN(pCar->_accelCmd, 0.3);
...
//				GfOut( "****** PIT_ASKED\n" );
			...
			case PIT_ASKED:
					...
					(pCar->ctrl.raceCmd & RM_CMD_PIT_ASKED) )
				{
					...
					double	fuel = distToRace * fuelPerM * 1.05 + (fuelPerLap - pCar->_fuel) + 1.0;
