/***************************************************************************
    ...
    version              : $Id: usr_36GP.cpp,v 1.1 2008/02/11 00:53:10 andrew Exp $
...
extern "C" int moduleWelcome(const tModWelcomeIn* welcomeIn, tModWelcomeOut* welcomeOut)
{
    ...
    if (strncmp(robotName,"usr_36GP",strlen("usr_36GP")) == 0)
		SetUpUSR_36GP();
	else 
		SetUpUSR();
...
extern "C" int usr_36GP(tModInfo *modInfo)
{
  ...
  snprintf(pathBuffer, BUFSIZE, "drivers/usr_36GP/usr_36GP.xml");
  snprintf(nameBuffer, BUFSIZE, "usr_36GP");
