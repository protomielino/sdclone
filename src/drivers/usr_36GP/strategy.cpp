/***************************************************************************
    ...
    version              : $Id: strategy.cpp,v 1.1 2008/02/11 00:53:10 andrew Exp $
 ...
 //if (car->_dammage < PIT_DAMMAGE)
 {
  ...
  for (int i = 0; i < opp->getNOpponents(); i++)
  {
   ...
   if (ocar->_pos < pos && ocar->_pos > car->_pos)
   {
    if (inpit)
    {
     float mytime = (tdble)((car->_distFromStartLine / track->length) * car->_lastLapTime + (car->_laps - ocar->_laps) * car->_bestLapTime);
     float othertime = (tdble)((ocar->_distFromStartLine / track->length) * ocar->_bestLapTime);
