...
static const string defaultBotName[MAXNBBOTS] = {
...
static const string defaultBotDesc[MAXNBBOTS] = {
  "kilo 1",  "kilo 2",  "kilo 3",  "kilo 4",  "kilo 5",
  "kilo 6",  "kilo 7",  "kilo 8",  "kilo 9",  "kilo 10", 
  "kilo 11", "kilo 12", "kilo 13", "kilo 14", "kilo 15",
  "kilo 16", "kilo 17", "kilo 18", "kilo 19", "kilo 20" 
...
static string nameBuffer;   //Robot's name
static string pathBuffer;   //Robot's xml-filename
...
const string sUndefined = "undefined";
...
extern "C" int moduleWelcome(const tModWelcomeIn* welcomeIn, tModWelcomeOut* welcomeOut)
{
  ...
  if(pRobotSettings) {  //robot settings XML could be read
    ...
    const string sDriverName = GfParmGetStrNC(pRobotSettings,
            ssBuf.str().c_str(), ROB_ATTR_NAME, (char*)sUndefined.c_str());

    // Check wether index 0 is used as start index
    if(sDriverName != sUndefined) { // Teams xml file uses index 0, 1, ..., N - 1
      indexOffset = 0;
    } else {  // Teams xml file uses index 1, 2, ..., N
      indexOffset = 1;
    }
    ...
    for(int i = 0; i < MAXNBBOTS; i++) {
      ...
      ssBuf << ROB_SECT_ROBOTS << "/" << ROB_LIST_INDEX << "/" << i + indexOffset;

      string sDriverName = GfParmGetStr(pRobotSettings, ssBuf.str().c_str(), ROB_ATTR_NAME, sUndefined.c_str());
      if(sDriverName != sUndefined) { // This driver is defined in robot's xml-file
        string sDriverDesc = GfParmGetStr(pRobotSettings, ssBuf.str().c_str(), ROB_ATTR_DESC, defaultBotDesc[i].c_str());
...
extern "C" int kilo2008(tModInfo *modInfo)
{
  ...
  if(pRobotSettings) {  //Let's look what we have to provide here
    ...
    for(int i = 0; i < NBBOTS; i++) {
      string sDriverName = GfParmGetStr(pRobotSettings, ssBuf.str().c_str(), ROB_ATTR_NAME, defaultBotName[i].c_str());
      string sDriverDesc = GfParmGetStr(pRobotSettings, ssBuf.str().c_str(), ROB_ATTR_DESC, defaultBotDesc[i].c_str());
      Drivers.push_back(make_pair(sDriverName, sDriverDesc));
    }//for i
  }//if RobotSettings
