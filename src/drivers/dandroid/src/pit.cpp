...
void Pit::init(PTrack t, PSituation s, PtCarElt c, int pitdamage, double pitentrymargin, bool HASTYC)
{
    if (pitdamage)
    {
    ...
    mypit = car->_pit;
    pitinfo = &track->pits;
    pitstop = inpitlane = penalty = false;
    fuelchecked = false;
    ...
    if (mypit != NULL)
    {
        ...
        {
            dt[i].y = sign * (fabs(pitinfo->driversPits->pos.toMiddle) - 0.3 * pitinfo->width - 2.2);
        }

        p[3].y = fabs(pitinfo->driversPits->pos.toMiddle) * sign;
        spline.newSpline(NPOINTS, p);
        dtspline.newSpline(DTPOINTS, dt);
    }
}
...
double Pit::getPitOffset(double fromstart)
{
    if (mypit != NULL)
    {
        if (getInPit() || (getPitstop() || getPenalty() && isBetween(fromstart)))
        {
...
double Pit::getFuel()
{
    ...
    LogDANDROID.info("fromStart:%g laps:%g lapsBehindLeader:%d fueltoend:%g pitstops:%d stintfuel:%g fuel:%g\n", mFromStart, laps, car->_lapsBehindLeader, fueltoend, pitstops, stintfuel, fuel);
