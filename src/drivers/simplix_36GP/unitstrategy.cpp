  ...
  (PTrack Track, PCarSettings *CarSettings, PSituation Situation, float Fuel)
{
  ...
  oMaxFuel =
	GfParmGetNum(*CarSettings,
	(char *) TDriver::SECT_PRIV,                 // Maximal möglicher
	(char *) PRV_MAX_FUEL,(char*) NULL,oMaxFuel);//   Tankinhalt
  GfOut("oMaxFuel (private) = %.1f\n",oMaxFuel);

  oStartFuel =
	GfParmGetNum(*CarSettings,
	(char *) TDriver::SECT_PRIV,                 // Tankinhalt beim Start
	(char *) PRV_START_FUEL,
	(char*) NULL,oStartFuel);
	  ...
	  && (oStartFuel > 0))
  {
    ...
    GfParmSetNum(*CarSettings,
      (char *) SECT_CAR,(char *) PRM_FUEL,       // Gewünschte Tankfüllung
  ...
  oMinLaps = (int)
	GfParmGetNum(*CarSettings,
	  (char *) TDriver::SECT_PRIV,               // Mindestanzahl an Runden
	  (char *) PRV_MIN_LAPS,
	  (char*) NULL,(float) oMinLaps);            //   die mit dem Tankinhalt
  ...
  GfParmSetNum(*CarSettings, (char *) SECT_CAR,
