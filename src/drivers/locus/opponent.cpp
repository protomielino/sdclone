...
#ifdef OPP_DEBUG
fprintf(stderr,"LOCUS >>%s %.3f (>%.3f <%.3f) ",car->_name,distance,-BACKCOLLDIST,FRONTCOLLDIST);
#endif
...
fprintf(stderr," FRONTSLOW\n");
		...
		// Is opponent behind and faster.
	 	else if (distance < -SIDECOLLDIST && getSpeed() > driver->getSpeed() - SPEED_PASS_MARGIN) 
		{
#ifdef OPP_DEBUG
fprintf(stderr," BACK\n");
#endif
...
void Opponent::updateOverlapTimer(tSituation *s, tCarElt *mycar)
{
	if (car->race.laps > mycar->race.laps ||
			((team == TEAM_FRIEND) && mycar->_dammage > car->_dammage + 1000)) 
	{
